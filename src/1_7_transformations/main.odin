...
init :: proc "c" () {
    ...
    // shader
    state.shader = sg.make_shader(transformations_shader_desc(sg.query_backend()))
    ...
    state.pipe = sg.make_pipeline({
        ...
        layout     = {
            attrs = {
                ATTR_transformations_position  = { format = .FLOAT3 },
                ATTR_transformations_tex_coord = { format = .FLOAT2 },
            },
        },
...
frame :: proc "c" () {
    ...
    dt := f32(state.timer.frame_time)

    // keys
    if key_code[.ESCAPE] do sapp.request_quit()
    if key_code[.W] do fs_params.mix_value += 0.02
    if key_code[.S] do fs_params.mix_value -= 0.02

    // fragment shader params
    fs_params.mix_value = clamp(fs_params.mix_value, 0, 1)
    ...
    p := linalg.matrix4_perspective_f32(80, aspect, 0.01, 1000)

    m := linalg.matrix4_translate_f32({ 0.0, 0.0, -2.0 }) * linalg.matrix4_from_yaw_pitch_roll_f32(0, 0, state.rotation)
...
update_timer :: proc(t: ^Timer) {
    t.current    = time.tick_since(t.start)
    t.frame_time = time.duration_seconds(t.current - t.previous)
    t.frame_time = clamp(t.frame_time, 0, MAX_DELTATIME)
